...
namespace SharpEcommerce.Core.Specifications
{
    ...
    {
        public ProductsWithTypesAndBrandSpecification(string sort, int? brandId, int? typeId) : base(x => (!brandId.HasValue || x.ProductBrandId == brandId) && (!typeId.HasValue || x.ProductTypeId == typeId))
        {
